"""
elevation/thueringen.py -- Höhenmodell Thüringen.

Behörde : TLBG (Thüringer Landesamt für Bodenmanagement und Geoinformation)
Portal  : https://geoportal.thueringen.de/
Produkt : DGM1, 1 m Gitterweite, GeoTIFF-Kacheln
CRS     : EPSG:25832
Lizenz  : Datenlizenz Deutschland -- Namensnennung 2.0 (Open Data seit 2024)

DGM1 als Open Data über das Geoportal Thüringen. UTM-Zone 32.

Vorgehen: Kacheln der Zielregion einmal herunterladen, Verzeichnis in
config.yaml eintragen:

    elevation:
      enabled: true
      source: state
      state: thueringen
      states:
        thueringen:
          path: data/dem/thueringen
          pattern: "*.tif"

Die Download-Links der Länder ändern sich regelmäßig; sie stehen deshalb
bewusst nicht im Code.
"""

from __future__ import annotations

from .common import LocalTileSource, WcsSource

SOURCE_INFO = {
    "state": "thueringen",
    "label": "Thüringen",
    "agency": "TLBG (Thüringer Landesamt für Bodenmanagement und Geoinformation)",
    "portal": "https://geoportal.thueringen.de/",
    "product": "DGM1",
    "resolution_m": 1.0,
    "crs": "EPSG:25832",
    "licence": "dl-de/by-2-0",
    "notes": "DGM1 als Open Data über das Geoportal Thüringen. UTM-Zone 32.",
}


def make_source(cfg: dict):
    """Baut die Höhenquelle für Thüringen aus der config."""
    scfg = ((cfg.get("elevation", {}) or {}).get("states", {}) or {}).get("thueringen", {}) or {}
    if scfg.get("wcs_url"):
        return WcsSource(
            url=str(scfg["wcs_url"]),
            coverage_id=str(scfg.get("coverage_id", "dgm1")),
            resolution_m=float(scfg.get("resolution_m", 1.0)),
            crs=str(scfg.get("crs", SOURCE_INFO["crs"])),
            name="thueringen_wcs",
            extra=scfg.get("wcs_extra", {}) or {},
        )
    return LocalTileSource(
        path=str(scfg.get("path", "data/dem/thueringen")),
        pattern=str(scfg.get("pattern", "*.tif")),
        resolution_m=float(scfg.get("resolution_m", 1.0)),
        name="thueringen_dgm1",
        src_crs=scfg.get("crs", SOURCE_INFO["crs"]),
    )
